<nav x-data="{ open: false }" class="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur-lg border-b border-surface-200/60">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <a href="/" class="flex items-center gap-2.5">
                <svg class="w-8 h-8 text-brand-600" fill="none" viewBox="0 0 32 32" stroke="currentColor" stroke-width="2" aria-hidden="true">
            ...
            {{-- Mobile Menu Button --}}
            <button @click="open = !open" :aria-expanded="open.toString()" aria-controls="mobile-menu" class="md:hidden p-2 rounded-lg text-surface-500 hover:bg-surface-100" aria-label="Toggle navigation menu">
    ...
    {{-- Mobile Menu --}}
    <div id="mobile-menu" x-show="open" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1" class="md:hidden border-t border-surface-200 bg-white">
